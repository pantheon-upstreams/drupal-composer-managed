...
/**
 * Class Hearsay Team Controller.
 */

class HSTeamController extends ControllerBase
{
    /**
     * Get Team Controller Theme IDs
    ...
    /**
     * Get Profile Data for Team Theme
     ...
     */
    public function getProfileDataForTeam($profileData, $moduleName)
    {
        $moduleData = [];
        ...
        if ($subOffices) {
            foreach ($subOffices as $office) {
                if (isset($office->office_title)) {
                    $address = [];
                    $latitude = $longitude = $json = '';
                    $officeTitle = isset($office->office_title) ? $office->office_title : '';
                    $officeImage = isset($office->office_image) ? $office->office_image : '';
                    $officeHours = isset($office->office_hours) ? $office->office_hours : '';
                    $address = [
    ...
    /**
     * Get Custom Content section for Team Theme
     * @param array Profile Data received form API Response
     * @param object Settings form object
    ...
    /**
     * Get Team Member section for Team Theme
     * @param array Profile Data received form API Response
     * @param object Settings form object
            ...
            foreach ($profileData->team_members as $teamMember) {
                ...
                if ($teamMember->associated_office === "0" || $teamMember->associated_office === "" || $teamMember->associated_office === "null" || $teamMember->associated_office === "Main Office") {
                    ...
                    switch ($teamMember->associated_office) {
                        case 1:
                            $profileData->suboffices[0] = $profileData->suboffices[0];
                            ...
                            break;
                        case 2:
                            $profileData->suboffices[1] = $profileData->suboffices[1];
                            ...
                            break;
                        case 3:
                            $profileData->suboffices[2] = $profileData->suboffices[2];
                            ...
                            break;
                        case 4:
                            $profileData->suboffices[3] = $profileData->suboffices[3];
                            ...
                            break;
                        case 5:
                            $profileData->suboffices[4] = $profileData->suboffices[4];
                            ...
                            break;
                        case 6:
                            $profileData->suboffices[5] = $profileData->suboffices[5];
                            ...
                            break;
                        case 7:
                            $profileData->suboffices[6] = $profileData->suboffices[6];
                            ...
                            break;
                        case 8:
                            $profileData->suboffices[7] = $profileData->suboffices[7];
                            ...
                            break;
                        case 9:
                            $profileData->suboffices[8] = $profileData->suboffices[8];
    ...
    /**
     * Get Footer section for Team Theme
     * @param array Profile Data received form API Response
     * @param object Settings form object
    ...
    public function getFooterData($profileData, $config)
    {
        ...
        $moduleData = [
            ...
            'disclaimer' => $profileData->disclaimer != '' ? $profileData->disclaimer : '',
    ...
    public function getHeroBannerData($profileData, $config)
    {
        $nodeType = $this->hearsayCommon->getThemeIdByNode()['node_type'];
        if ($nodeType == 'home') {
            $defaultBannerBgName = $config->get('header_banner_image') ?? '';
            $defaultBannerBgDetails = $this->hearsayCommon->getCanonicalMediaDetailsByTheme($defaultBannerBgName, 'field_banner_canonical_name', HS_LIBRARY_TEAM_AB);
            $defaultBannerBgMedia = reset($defaultBannerBgDetails);
            $defaultBannerBgUrl = $this->hearsayCommon->getMediaImageUrl($defaultBannerBgMedia->field_media_image->target_id); // Get media image URL

            $bannerBgName = $profileData->profile_background[0] ?? null;
            $bannerBgDetails = $this->hearsayCommon->getCanonicalMediaDetailsByTheme($bannerBgName, 'field_banner_canonical_name', HS_LIBRARY_TEAM_AB);
            $bannerBgMedia = reset($bannerBgDetails);
            $bannerBgUrl = $bannerBgMedia ? $this->hearsayCommon->getMediaImageUrl($bannerBgMedia->field_media_image->target_id) : null; // Get media image URL
            ...
            return [
                'theme' => HS_LIBRARY_TEAM_AB,
                'node_type' => $nodeType,
                'bg_image' => isset($bannerBgName) ? $bannerBgUrl : $defaultBannerBgUrl,
                'team_name' => $profileData->team_name ?? ($profileData->first_name . ' ' . $profileData->last_name),
                'professional_summary' => $profileData->professional_summary ?? '',
                'appointments' => $appointments ?? '',
                'open_new_window' => $profileData->schedule_appointment_link->open_in_new_window ?? '',
                'profile_photo' => $profileData->profile_photo ?? '',
                'networks' => $profileData->networks,
            ];
        } elseif ($nodeType == 'solutions') {
            $solutionBannerName = $config->get('solution_image') != '' ? $config->get('solution_image') : '';
            $solutionBannerDetails = $this->hearsayCommon->getCanonicalMediaDetailsByTheme($solutionBannerName, 'field_banner_canonical_name', HS_LIBRARY_TEAM_AB);
    ...
    /**
     * Get Products section for Team Theme
     * @param array Profile Data received form API Response
     * @param object Settings form object
    ...
    /**
     * Get Privacy Policy section for Team Theme
     * @param array Profile Data received form API Response
     * @param object Settings form object
    ...
    /**
     * get Insights Data for Team Theme
    ...
    {
        $moduleData = $insightsResponseData = $insightsData = [];
        $nodeType = $this->hearsayCommon->getThemeIdByNode()['node_type'];
